...
router.post("/barbershop", Cloudinary.single("picture"), isLoggedIn, (req, res, next) => {
  let {
    name,
    gender,
    streetAddress,
  ...
  BarberShop.findById(req.params.barbershopId)
    .then(barbershop => {
      if(barbershop) {
        ...
        next({
          status: 400,
          message:'There is no Barber Shop with the id: '+ req.params.barbershopId
        })
      }
    })
    .catch(err => next(err))
...
router.delete("/delete-barbershop", isLoggedIn, (req,res,next) => {
  BarberShop.findOneAndRemove(req.params._id)
    .then(barbershop => {
      res.json({
        success: true,
      })
